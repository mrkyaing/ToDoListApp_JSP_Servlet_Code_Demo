...
public class TodoDaoServices implements ITodoDao {
	...
	public TodoDaoServices() {
		System.out.println("create the TodoDaoService");
	}

	@Override
	public void insertTodo(TodoEntity todo) throws SQLException {
	...
	@Override
	public TodoEntity selectTodo(long todoId) {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (rs.next()) {
				...
				todo = new TodoEntity(id, title, username, description, targetDate, isDone);
			}
		} catch (SQLException exception) {
			JDBCUtils.printSQLException(exception);
		}
	...
	public List<TodoEntity> selectAllTodos() {
		...
		List<TodoEntity> todosList = new ArrayList<>();
		// Step 1: Establishing a Connection
		try (Connection connection = JDBCUtils.getConnection();
		// Step 2:Create a statement using connection object
		...
		ResultSet rs = preparedStatement.executeQuery();
			...
			while (rs.next()) {
				...
				LocalDate targetDate = rs.getDate("target_date").toLocalDate();
				boolean isDone = rs.getBoolean("is_done");
				todosList.add(new TodoEntity(id, title, username, description, targetDate, isDone));
			}
		} catch (SQLException exception) {
			JDBCUtils.printSQLException(exception);
		}
	...
	@Override
	public boolean updateTodo(TodoEntity todo) throws SQLException {
		boolean rowUpdated;
		try (Connection connection = JDBCUtils.getConnection();
